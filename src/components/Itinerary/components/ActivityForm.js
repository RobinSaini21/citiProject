...
function ActivityForm({ idx, length, recreate, destroy }) {
  ...
  const [hastags, setHastages] = useState([]);
  ...
  const handleSubmit = (val) => {
    // if (hastags.length < 5) {
    //   setValidate(value => ({ ...value, validateHasTags: "Must Add 5 Or More Tags" }));
    // }
    // if (hastags.length === 0) {
    //   setValidate((value) => ({ ...value, validateHasTags: "Required" }));
    // } else if (Object.keys(address).length === 0) {
    //   setValidate((value) => ({ ...value, validateAddress: "Required" }));
    // } else {
    //   setValidate({ ...validate, validateHasTags: "" });
    //   setValidate({ ...validate, validateAddress: "" });

    // const allvalues = { val, hastags, address };
    setTimeout(() => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      friends: Yup.array()
        .of(
          Yup.object().shape({
            hastag_: Yup.string().min(4, "too short").required("Required"),
          })
        )
        .required("Required")
        .required("Must have 5 Tags") // these constraints are shown if and only if inner constraints are satisfied
        .min(5, "Minimum of 5 Tags"),
                              ...
                              formik.values.friends.map((friend, index) => {
                                ...
                                return (
                                  <>
                                    ...
                                    >
                                      {formik.values.friends[index].hastag_}
                                      <img src="/img/close_icon.svg" alt="" />
                                    </a>
                                    {formik.errors.friends &&
                                      formik.errors.friends[index].hastag_ &&
                                      formik.errors.friends[index].hastag_ &&
                                      formik.touched.friends &&
                                      formik.touched.friends[index].hastag_ && (
